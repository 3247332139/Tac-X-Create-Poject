...
public class TextureGlyphProvider implements IGlyphProvider {
   ...
   public static class Factory implements IGlyphProviderFactory {
      ...
      public static TextureGlyphProvider.Factory fromJson(JsonObject pJson) {
         int i = JSONUtils.getAsInt(pJson, "height", 8);
         int j = JSONUtils.getAsInt(pJson, "ascent");
         if (j > i) {
            throw new JsonParseException("Ascent " + j + " higher than height " + i);
         } else {
            List<int[]> list = Lists.newArrayList();
            JsonArray jsonarray = JSONUtils.getAsJsonArray(pJson, "chars");
            ...
            if (!list.isEmpty() && ((int[])list.get(0)).length != 0) {
               return new TextureGlyphProvider.Factory(new ResourceLocation(JSONUtils.getAsString(pJson, "file")), i, j, list);
            } else {
      ...
      public IGlyphProvider create(IResourceManager pResourceManager) {
         try (IResource iresource = pResourceManager.getResource(this.texture)) {
