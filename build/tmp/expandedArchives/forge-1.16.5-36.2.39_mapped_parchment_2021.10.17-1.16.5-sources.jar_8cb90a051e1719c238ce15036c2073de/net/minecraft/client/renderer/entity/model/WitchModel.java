...
public class WitchModel<T extends Entity> extends VillagerModel<T> {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      this.nose.xRot = MathHelper.sin((float)pEntity.tickCount * f) * 4.5F * ((float)Math.PI / 180F);
      this.nose.yRot = 0.0F;
      this.nose.zRot = MathHelper.cos((float)pEntity.tickCount * f) * 2.5F * ((float)Math.PI / 180F);
      if (this.holdingItem) {
