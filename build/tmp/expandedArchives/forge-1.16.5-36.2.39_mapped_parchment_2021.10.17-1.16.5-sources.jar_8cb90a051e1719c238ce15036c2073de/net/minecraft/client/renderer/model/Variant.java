...
public class Variant implements IModelTransform {
   ...
   public static class Deserializer implements JsonDeserializer<Variant> {
      ...
      private boolean getUvLock(JsonObject pJson) {
         return JSONUtils.getAsBoolean(pJson, "uvlock", false);
      }

      protected ModelRotation getBlockRotation(JsonObject pJson) {
         int i = JSONUtils.getAsInt(pJson, "x", 0);
         int j = JSONUtils.getAsInt(pJson, "y", 0);
      ...
      protected ResourceLocation getModel(JsonObject pJson) {
         return new ResourceLocation(JSONUtils.getAsString(pJson, "model"));
      }

      protected int getWeight(JsonObject pJson) {
         int i = JSONUtils.getAsInt(pJson, "weight", 1);
         if (i < 1) {
