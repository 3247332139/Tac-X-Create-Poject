...
public class Multipart implements IUnbakedModel {
   ...
   public Collection<RenderMaterial> getMaterials(Function<ResourceLocation, IUnbakedModel> pModelGetter, Set<Pair<String, String>> pMissingTextureErrors) {
      return this.getSelectors().stream().flatMap((p_228832_2_) -> {
         return p_228832_2_.getVariant().getMaterials(pModelGetter, pMissingTextureErrors).stream();
      }).collect(Collectors.toSet());
   }
   ...
   public IBakedModel bake(ModelBakery pModelBakery, Function<RenderMaterial, TextureAtlasSprite> pSpriteGetter, IModelTransform pTransform, ResourceLocation pLocation) {
      ...
      for(Selector selector : this.getSelectors()) {
         IBakedModel ibakedmodel = selector.getVariant().bake(pModelBakery, pSpriteGetter, pTransform, pLocation);
         if (ibakedmodel != null) {
