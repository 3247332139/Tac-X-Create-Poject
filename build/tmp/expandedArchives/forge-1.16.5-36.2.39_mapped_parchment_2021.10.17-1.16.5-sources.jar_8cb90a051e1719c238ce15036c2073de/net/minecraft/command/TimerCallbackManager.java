...
public class TimerCallbackManager<T> {
   ...
   public void tick(T p_216331_1_, long pGameTime) {
      while(true) {
         TimerCallbackManager.Entry<T> entry = this.queue.peek();
         if (entry == null || entry.triggerTime > pGameTime) {
            return;
         }

         this.queue.remove();
         this.events.remove(entry.id, pGameTime);
         entry.callback.handle(p_216331_1_, this, pGameTime);
      }
   }
