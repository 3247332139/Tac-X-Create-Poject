...
public class GatherPOITask extends Task<CreatureEntity> {
   ...
   protected void start(ServerWorld pLevel, CreatureEntity pEntity, long pGameTime) {
      ...
      this.batchCache.long2ObjectEntrySet().removeIf((p_241362_2_) -> {
         return !p_241362_2_.getValue().isStillValid(pGameTime);
      });
      Predicate<BlockPos> predicate = (p_220603_3_) -> {
         ...
         if (gatherpoitask$retrymarker == null) {
            return true;
         } else if (!gatherpoitask$retrymarker.shouldRetry(pGameTime)) {
            return false;
         } else {
            gatherpoitask$retrymarker.markAttempt(pGameTime);
            return true;
         }
      };
      Set<BlockPos> set = pointofinterestmanager.findAllClosestFirst(this.poiType.getPredicate(), predicate, pEntity.blockPosition(), 48, PointOfInterestManager.Status.HAS_SPACE).limit(5L).collect(Collectors.toSet());
      ...
      if (path != null && path.canReach()) {
         ...
         pointofinterestmanager.getType(blockpos1).ifPresent((p_225441_5_) -> {
            ...
            this.onPoiAcquisitionEvent.ifPresent((p_242291_2_) -> {
               pLevel.broadcastEntityEvent(pEntity, p_242291_2_);
            });
            this.batchCache.clear();
            DebugPacketSender.sendPoiTicketCountPacket(pLevel, blockpos1);
         });
      } else {
         for(BlockPos blockpos : set) {
            this.batchCache.computeIfAbsent(blockpos.asLong(), (p_241363_3_) -> {
               return new GatherPOITask.RetryMarker(pEntity.level.random, pGameTime);
            });
         }
      }
