   ...
   protected void start(ServerWorld pLevel, LivingEntity pEntity, long pGameTime) {
      ...
      if (blockstate.is(BlockTags.WOODEN_DOORS)) {
         ...
         if (!doorblock.isOpen(blockstate)) {
            doorblock.setOpen(pLevel, blockstate, blockpos, true);
         }
      ...
      if (blockstate1.is(BlockTags.WOODEN_DOORS)) {
         ...
         if (!doorblock1.isOpen(blockstate1)) {
            doorblock1.setOpen(pLevel, blockstate1, blockpos1, true);
            this.rememberDoorToClose(pLevel, pEntity, blockpos1);
         }
      }

      closeDoorsThatIHaveOpenedOrPassedThrough(pLevel, pEntity, pathpoint, pathpoint1);
   }
