...
public class SUpdateLightPacket implements IPacket<IClientPlayNetHandler> {
   ...
   public void write(PacketBuffer pBuffer) throws IOException {
      ...
      pBuffer.writeVarInt(this.skyYMask);
      pBuffer.writeVarInt(this.blockYMask);
      pBuffer.writeVarInt(this.emptySkyYMask);
      pBuffer.writeVarInt(this.emptyBlockYMask);

      for(byte[] abyte : this.skyUpdates) {
         pBuffer.writeByteArray(abyte);
      }

      for(byte[] abyte1 : this.blockUpdates) {
         pBuffer.writeByteArray(abyte1);
      }
