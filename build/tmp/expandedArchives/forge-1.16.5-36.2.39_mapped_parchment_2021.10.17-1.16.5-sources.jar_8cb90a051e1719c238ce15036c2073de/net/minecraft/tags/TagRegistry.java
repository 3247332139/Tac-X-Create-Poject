...
public class TagRegistry<T> {
   ...
   public ITag.INamedTag<T> bind(String pId) {
       return add(new TagRegistry.NamedTag<>(new ResourceLocation(pId)));
   }
   ...
   public void reset(ITagCollectionSupplier pSupplier) {
      ITagCollection<T> itagcollection = this.collectionGetter.apply(pSupplier);
   ...
   public Set<ResourceLocation> getMissingTags(ITagCollectionSupplier pSupplier) {
      ITagCollection<T> itagcollection = this.collectionGetter.apply(pSupplier);
   ...
   static class NamedTag<T> implements ITag.INamedTag<T> {
      ...
      private NamedTag(ResourceLocation pName) {
         this.name = pName;
      }
      ...
      void rebind(Function<ResourceLocation, ITag<T>> pIdToTagFunction) {
