...
public class TrackedEntity {
   ...
   public void sendPairingData(Consumer<IPacket<?>> pPacketConsumer) {
      ...
      pPacketConsumer.accept(ipacket);
      if (!this.entity.getEntityData().isEmpty()) {
         pPacketConsumer.accept(new SEntityMetadataPacket(this.entity.getId(), this.entity.getEntityData(), true));
      }
      ...
      if (this.entity instanceof LivingEntity) {
         ...
         if (!collection.isEmpty()) {
            pPacketConsumer.accept(new SEntityPropertiesPacket(this.entity.getId(), collection));
         }
      ...
      if (flag && !(ipacket instanceof SSpawnMobPacket)) {
         pPacketConsumer.accept(new SEntityVelocityPacket(this.entity.getId(), this.ap));
      }
      ...
      if (this.entity instanceof LivingEntity) {
         ...
         if (!list.isEmpty()) {
            pPacketConsumer.accept(new SEntityEquipmentPacket(this.entity.getId(), list));
         }
      }
      ...
      if (this.entity instanceof LivingEntity) {
         ...
         for(EffectInstance effectinstance : livingentity.getActiveEffects()) {
            pPacketConsumer.accept(new SPlayEntityEffectPacket(this.entity.getId(), effectinstance));
         }
      }

      if (!this.entity.getPassengers().isEmpty()) {
         pPacketConsumer.accept(new SSetPassengersPacket(this.entity));
      }

      if (this.entity.isPassenger()) {
         pPacketConsumer.accept(new SSetPassengersPacket(this.entity.getVehicle()));
      }

      if (this.entity instanceof MobEntity) {
         MobEntity mobentity = (MobEntity)this.entity;
         if (mobentity.isLeashed()) {
            pPacketConsumer.accept(new SMountEntityPacket(mobentity, mobentity.getLeashHolder()));
         }
      }
   ...
   private void broadcastAndSend(IPacket<?> pPacket) {
      this.broadcast.accept(pPacket);
      if (this.entity instanceof ServerPlayerEntity) {
         ((ServerPlayerEntity)this.entity).connection.send(pPacket);
      }
