...
public class DimensionGeneratorSettings {
   ...
   public DimensionGeneratorSettings(long pSeed, boolean pGenerateFeatures, boolean pGenerateBonusChest, SimpleRegistry<Dimension> pDimensions) {
      this(pSeed, pGenerateFeatures, pGenerateBonusChest, pDimensions, Optional.empty());
      Dimension dimension = pDimensions.get(Dimension.OVERWORLD);
      if (dimension == null) {
   ...
   public static DimensionGeneratorSettings demoSettings(DynamicRegistries pRegistry) {
      Registry<Biome> registry = pRegistry.registryOrThrow(Registry.BIOME_REGISTRY);
      int i = "North Carolina".hashCode();
      Registry<DimensionType> registry1 = pRegistry.registryOrThrow(Registry.DIMENSION_TYPE_REGISTRY);
      Registry<DimensionSettings> registry2 = pRegistry.registryOrThrow(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY);
   ...
   public static DimensionGeneratorSettings makeDefault(Registry<DimensionType> pDimensionTypes, Registry<Biome> pBiomes, Registry<DimensionSettings> pSettings) {
      long i = (new Random()).nextLong();
      return new DimensionGeneratorSettings(i, true, false, withOverworld(pDimensionTypes, DimensionType.defaultDimensions(pDimensionTypes, pBiomes, pSettings, i), makeDefaultOverworld(pBiomes, pSettings, i)));
   }

   public static NoiseChunkGenerator makeDefaultOverworld(Registry<Biome> pBiomes, Registry<DimensionSettings> pSettings, long pSeed) {
      return new NoiseChunkGenerator(new OverworldBiomeProvider(pSeed, false, false, pBiomes), pSeed, () -> {
         return pSettings.getOrThrow(DimensionSettings.OVERWORLD);
      });
   }
   ...
   public static SimpleRegistry<Dimension> withOverworld(Registry<DimensionType> pDimensionTypes, SimpleRegistry<Dimension> pDimensions, ChunkGenerator pChunkGenerator) {
      Dimension dimension = pDimensions.get(Dimension.OVERWORLD);
      Supplier<DimensionType> supplier = () -> {
         return dimension == null ? pDimensionTypes.getOrThrow(DimensionType.OVERWORLD_LOCATION) : dimension.type();
      };
      return withOverworld(pDimensions, supplier, pChunkGenerator);
   }

   public static SimpleRegistry<Dimension> withOverworld(SimpleRegistry<Dimension> pDimensions, Supplier<DimensionType> pDimensionTypes, ChunkGenerator pChunkGenerator) {
      SimpleRegistry<Dimension> simpleregistry = new SimpleRegistry<>(Registry.LEVEL_STEM_REGISTRY, Lifecycle.experimental());
      simpleregistry.register(Dimension.OVERWORLD, new Dimension(pDimensionTypes, pChunkGenerator), Lifecycle.stable());

      for(Entry<RegistryKey<Dimension>, Dimension> entry : pDimensions.entrySet()) {
         RegistryKey<Dimension> registrykey = entry.getKey();
         if (registrykey != Dimension.OVERWORLD) {
            simpleregistry.register(registrykey, entry.getValue(), pDimensions.lifecycle(entry.getValue()));
         }
      }
   ...
   public static DimensionGeneratorSettings create(DynamicRegistries pRegistry, Properties pProperties) {
      ...
      Registry<DimensionType> registry2 = pRegistry.registryOrThrow(Registry.DIMENSION_TYPE_REGISTRY);
      Registry<Biome> registry = pRegistry.registryOrThrow(Registry.BIOME_REGISTRY);
      Registry<DimensionSettings> registry1 = pRegistry.registryOrThrow(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY);
      ...
      if (type != null) return type.createSettings(pRegistry, i, flag, false, s);
      switch(s4) {
      case "flat":
   ...
   @OnlyIn(Dist.CLIENT)
   public DimensionGeneratorSettings withSeed(boolean pHardcore, OptionalLong pLevelSeed) {
      ...
      if (this.isDebug()) {
         ...
         dimensiongeneratorsettings = new DimensionGeneratorSettings(i, this.generateFeatures(), this.generateBonusChest() && !pHardcore, simpleregistry);
      }
