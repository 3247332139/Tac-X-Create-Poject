...
public class SkyLightStorage extends SectionLightStorage<SkyLightStorage.StorageMap> {
   ...
   protected int getLightValue(long pLevelPos) {
      ...
      if (k != skylightstorage$storagemap.currentLowestY && j < k) {
         ...
         if (nibblearray == null) {
            for(pLevelPos = BlockPos.getFlatIndex(pLevelPos); nibblearray == null; nibblearray = this.getDataLayer(skylightstorage$storagemap, i)) {
   ...
   protected void markNewInconsistencies(LightEngine<SkyLightStorage.StorageMap, ?> pEngine, boolean pUpdateSkyLight, boolean pUpdateBlockLight) {
