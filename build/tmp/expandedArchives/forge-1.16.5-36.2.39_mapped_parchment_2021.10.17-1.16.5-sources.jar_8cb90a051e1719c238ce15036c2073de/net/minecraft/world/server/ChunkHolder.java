...
public class ChunkHolder {
   ...
   public void sectionLightChanged(LightType pType, int pSectionY) {
      Chunk chunk = this.getTickingChunk();
      if (chunk != null) {
         chunk.setUnsaved(true);
         if (pType == LightType.SKY) {
            this.skyChangedLightSectionFilter |= 1 << pSectionY - -1;
         } else {
            this.blockChangedLightSectionFilter |= 1 << pSectionY - -1;
         }

      }
   }

   public void broadcastChanges(Chunk pChunk) {
      if (this.hasChangedSections || this.skyChangedLightSectionFilter != 0 || this.blockChangedLightSectionFilter != 0) {
         ...
         if (this.skyChangedLightSectionFilter != 0 || this.blockChangedLightSectionFilter != 0) {
            this.broadcast(new SUpdateLightPacket(pChunk.getPos(), this.lightEngine, this.skyChangedLightSectionFilter, this.blockChangedLightSectionFilter, true), !this.resendLight);
   ...
   private void broadcastBlockEntity(World pLevel, BlockPos pPos) {
   ...
   private void broadcast(IPacket<?> pPacket, boolean pBoundaryOnly) {
      this.playerProvider.getPlayers(this.pos, pBoundaryOnly).forEach((p_219304_1_) -> {
         p_219304_1_.connection.send(pPacket);
      });
   }
   ...
   public static ChunkStatus getStatus(int pLevel) {
      return pLevel < 33 ? ChunkStatus.FULL : ChunkStatus.getStatus(pLevel - 33);
   }

   public static ChunkHolder.LocationType getFullChunkStatus(int pLevel) {
      return FULL_CHUNK_STATUSES[MathHelper.clamp(33 - pLevel + 1, 0, FULL_CHUNK_STATUSES.length - 1)];
   }
   ...
   public interface IPlayerProvider {
       ...
       */
      Stream<ServerPlayerEntity> getPlayers(ChunkPos pPos, boolean pBoundaryOnly);
   }
   ...
   public static enum LocationType {
      ...
      public boolean isOrAfter(ChunkHolder.LocationType pType) {
